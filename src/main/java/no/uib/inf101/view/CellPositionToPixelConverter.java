...
public class CellPositionToPixelConverter {
  ...
  public Rectangle2D getBoundsForCell(CellPosition cp) throws IllegalArgumentException {
    ...
    double cellWidth = (box.getWidth()) / cols - margin;
    double cellHeight = (box.getHeight()) / rows - margin;
    double cellX = box.getX() + margin / 2 + col * (margin + cellWidth);
    double cellY = box.getY() + margin / 2 + row * (margin + cellHeight);
